      ...
      <Row>
        <Col span="24">
          <Card>
            ...
            <Button type="success" style="margin-top: 20px" long @click="download(courseExam.resource)"
                    icon="ios-arrow-round-down" v-if="courseExam.resource !== ''">下载附件
            </Button>
          </Card>
        </Col>
      </Row>
      ...
      <Row style="margin-top: 20px">
        <Col span="24">
          ...
          <Row span="24" style="margin-top: 20px">
            <Col>
              <Button type="primary" long @click="save" :loading="$store.state.courseManage.saveLoding">保 存</Button>
            </Col>
          </Row>
        </Col>
      </Row>
  ...
  export default {
    ...
    methods: {
      ...
      save() {
        ...
        this.$store.dispatch("courseManage/doSaveStudentExam", {"examination": this.courseExam.id, "student": this.$store.state.loginManage.id, "title": this.form.title, "resource": this.form.resource, "content": this.form.content})
      },
      // 表单验证
      dataValidate(value) {
