<template>
  <Row style="margin-top: 20px">
    <Col span="22" offset="1">
      <Tabs value="name1">
        <TabPane label="待完成作业" name="name1">
          <Row>
            <Col span="4" style="padding: 5px" v-for="(value, index) in $store.state.courseManage.courseJobData.miss" :key="index">
              <Card>
        ...
        <TabPane label="已完成作业" name="name2">
          <Row>
            <Col span="4" style="padding: 5px" v-for="(value, index) in $store.state.courseManage.courseJobData.pass" :key="index">
              <Card>
                <p slot="title">{{value.title}}</p>
                <p>起始时间：{{moment(value.startTime).format('YYYY-MM-DD h:mm')}}</p>
                <p>结束时间：{{moment(value.endTime).format('YYYY-MM-DD h:mm')}}</p>
...
<script>
    ...
    export default {
        name: "index",
        mounted() {
          this.$store.dispatch("courseManage/doQueryStudentJobByCourseAndStudent", this.$route.params.id)
        },
        methods: {
          moment,
          watch(value) {
            this.$router.push("/learn/courseJobDetail/" + value)
          }
        }
    }
</script>
