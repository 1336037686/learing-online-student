      ...
      <Row>
        <Col span="24">
          <Card>
            ...
            <Button type="success" style="margin-top: 20px" long @click="download(courseJob.resource)" icon="ios-arrow-round-down" v-if="courseJob.resource !== ''">下载附件
            </Button>
          </Card>
        </Col>
      </Row>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.form = JSON.parse(JSON.stringify(this.$store.state.courseManage.studentJobDetail))
    },
    methods: {
      //下载
      download(url) {
      ...
      save() {
        ...
        this.$store.dispatch("courseManage/doUpdateStudentJob", {"id": this.form.id,"courseJob": this.courseJob.id, "student": this.$store.state.loginManage.id, "title": this.form.title, "resource": this.form.resource, "content": this.form.content})
      },
      // 表单验证
      dataValidate(value) {
